...
curl -XPOST 'localhost:9200/task1a/wikipage/_bulk?pretty' -H "Content-Type: application/json" --data-binary "@data/out.txt" 
...
curl -XPOST 'localhost:9200/task1b/wikipage/_bulk?pretty' -H "Content-Type: application/json" --data-binary "@data/out.txt" 
...
curl -XPOST 'localhost:9200/task2/wikipage/_bulk?pretty' -H "Content-Type: application/json" --data-binary "@data/out.txt" 
...
curl -XPUT "localhost:9200/task3b/?pretty" -H 'Content-Type: application/json' -d'
{
    ...
    "mappings" : {
        "task3b" : {
        	"properties" : {
            	"clicks" : {"type" : "long", "index" : "not_analyzed", "doc_values" : true}
        	}
        }
    }
}'

curl -XPOST 'localhost:9200/task3b/wikipage/_bulk?pretty' -H "Content-Type: application/json" --data-binary "@data/out.txt" 
